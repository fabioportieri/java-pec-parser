...
import it.dmi.mail.uudecoder.exception.model.UUDecodedBean;
...
public class UUDecoder {
    ...
    private static boolean internalContainsUUencodedAttachments(String content) throws UUDecoderException {
        ...
        try {
            ...
            // String lastLine = reader.lines().reduce((first, second) -> second).orElse(null); , jdk1.7 equivalent:
    ...
    private static String getUUDecodedFileName(String content, List<UUDecodedAttachment> res) {
        ...
        try {
            ...
            for (UUDecodedAttachment uda : res) {
                fileNames.add(uda.getFileName());
            }
            return getVersionedFileName("unnamed", fileNames, 1);
